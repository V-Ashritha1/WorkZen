...
public class AuthController {
    ...
    public ResponseEntity<AuthResponse> login(@Valid @RequestBody Login login){
        ...
        if (user.isPresent()){
            ...
            String token = jwtUtil.generateToken(presentUser.getId(), presentUser.getUsername(), presentUser.getRole());
            return ResponseEntity.ok(new AuthResponse(presentUser.getId(), presentUser.getUsername(), presentUser.getRole(), token));
        }
